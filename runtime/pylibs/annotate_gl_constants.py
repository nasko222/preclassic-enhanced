...
def annotate_constants(code):
    ...
    return annotate_constants_opengl(code)

def annotate_constants_input(code):
    def process_call(match):
        ...
        def expand_constant(match):
            ...
            try:
                if int(constant) not in _CONSTANTS_KEYS: return constant
            except Exception as e:
                return constant
            constant = int(constant)
            return 'Keyboard.' + '%s' % (_CONSTANTS_KEYS[constant])
        return _CONSTANT_REGEX.sub(expand_constant, full_call)
    return _INPUT_REGEX.sub(process_call, code)

def annotate_constants_opengl(code):
    def process_call(match):
        full_call = match.group(0)
